...
export class MarketTradesService {
  ...
  async findAll(args?: MarketTradesArgs): Promise<MarketTrades[]> {
    ...
    if (args && args.buyerOrderId !== undefined) where['buyerOrderId'] = args.buyerOrderId;
    if (args && args.sellerOrderId !== undefined) where['sellerOrderId'] = args.sellerOrderId;
    if (args && args.buyerProsumerId !== undefined) where['buyerProsumerId'] = args.buyerProsumerId;
    if (args && args.sellerProsumerId !== undefined) where['sellerProsumerId'] = args.sellerProsumerId;
    if (args && args.buyerWalletAddress !== undefined) where['buyerWalletAddress'] = args.buyerWalletAddress;
    if (args && args.sellerWalletAddress !== undefined) where['sellerWalletAddress'] = args.sellerWalletAddress;
    if (args && args.tradedEtkAmount !== undefined) where['tradedEtkAmount'] = args.tradedEtkAmount;
    ...
    if (args && args.tradeTimestamp !== undefined) where['tradeTimestamp'] = args.tradeTimestamp;
    if (args && args.gasFeeWei !== undefined) where['gasFeeWei'] = args.gasFeeWei;
  ...
  async create(input: CreateMarketTradesInput): Promise<MarketTrades> {
    ...
    if (input.tradeTimestamp) (createData as any).tradeTimestamp = new Date(input.tradeTimestamp);
    ...
    if (input.prosumers2Ids && input.prosumers2Ids.length > 0) {
      const prosumers2Entities = await this.ProsumersRepo.findBy({ prosumerId: In(input.prosumers2Ids) });
      (createData as any).prosumers2 = prosumers2Entities;
    }
    // Handle wallets2 relation
    if (input.wallets2Ids && input.wallets2Ids.length > 0) {
      const wallets2Entities = await this.WalletsRepo.findBy({ walletAddress: In(input.wallets2Ids) });
  ...
  async update(tradeId: any, input: CreateMarketTradesInput): Promise<MarketTrades> {
    const existing = await this.findOne(tradeId);
    
    // Convert input types to match entity types
    const updateData: Partial<MarketTrades> = { ...input } as any;
    
    if (input.tradeTimestamp) (updateData as any).tradeTimestamp = new Date(input.tradeTimestamp);
  ...
  async findProsumers(tradeId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { tradeId }, relations: ['prosumers'] });
  ...
  async findWallets(tradeId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { tradeId }, relations: ['wallets'] });
  ...
  async findProsumers2(tradeId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { tradeId }, relations: ['prosumers2'] });
  ...
  async findWallets2(tradeId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { tradeId }, relations: ['wallets2'] });
