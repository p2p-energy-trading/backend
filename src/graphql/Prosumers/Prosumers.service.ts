...
export class ProsumersService {
  ...
  async findAll(args?: ProsumersArgs): Promise<Prosumers[]> {
    ...
    if (args && args.passwordHash !== undefined) where['passwordHash'] = args.passwordHash;
    ...
    const relations = ['blockchainapprovalsList', 'devicecommandsList', 'idrsconversionsList', 'markettradesList', 'markettradesList2', 'smartmetersList', 'tradeorderscacheList', 'transactionlogsList', 'walletsList'];
    return this.repo.find({ where, relations });
  }

  async findOne(prosumerId: any): Promise<Prosumers> {
    const relations = ['blockchainapprovalsList', 'devicecommandsList', 'idrsconversionsList', 'markettradesList', 'markettradesList2', 'smartmetersList', 'tradeorderscacheList', 'transactionlogsList', 'walletsList'];
    const entity = await this.repo.findOne({ where: { prosumerId }, relations });
    if (!entity) {
  ...
  async update(prosumerId: any, input: CreateProsumersInput): Promise<Prosumers> {
  ...
  async findBlockchainapprovalsList(prosumerId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { prosumerId }, relations: ['blockchainapprovalsList'] });
    const entities = parent?.blockchainapprovalsList || [];
    // Convert entities to match GraphQL output format
    return entities.map(entity => ({
      ...entity,
      expiresAt: entity.expiresAt ? (entity.expiresAt instanceof Date ? entity.expiresAt.toISOString() : entity.expiresAt) : null,
      createdAt: entity.createdAt ? (entity.createdAt instanceof Date ? entity.createdAt.toISOString() : entity.createdAt) : null,
      confirmedAt: entity.confirmedAt ? (entity.confirmedAt instanceof Date ? entity.confirmedAt.toISOString() : entity.confirmedAt) : null,
    }));
  }

  async findDevicecommandsList(prosumerId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { prosumerId }, relations: ['devicecommandsList'] });
    const entities = parent?.devicecommandsList || [];
    // Convert entities to match GraphQL output format
    return entities.map(entity => ({
      ...entity,
      sentAt: entity.sentAt ? (entity.sentAt instanceof Date ? entity.sentAt.toISOString() : entity.sentAt) : null,
      acknowledgedAt: entity.acknowledgedAt ? (entity.acknowledgedAt instanceof Date ? entity.acknowledgedAt.toISOString() : entity.acknowledgedAt) : null,
      timeoutAt: entity.timeoutAt ? (entity.timeoutAt instanceof Date ? entity.timeoutAt.toISOString() : entity.timeoutAt) : null,
    }));
  }

  async findIdrsconversionsList(prosumerId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { prosumerId }, relations: ['idrsconversionsList'] });
  ...
  async findMarkettradesList(prosumerId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { prosumerId }, relations: ['markettradesList'] });
    const entities = parent?.markettradesList || [];
    // Convert entities to match GraphQL output format
    return entities.map(entity => ({
      ...entity,
      tradeTimestamp: entity.tradeTimestamp ? (entity.tradeTimestamp instanceof Date ? entity.tradeTimestamp.toISOString() : entity.tradeTimestamp) : null,
      createdAt: entity.createdAt ? (entity.createdAt instanceof Date ? entity.createdAt.toISOString() : entity.createdAt) : null,
    }));
  }

  async findMarkettradesList2(prosumerId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { prosumerId }, relations: ['markettradesList2'] });
  ...
  async findSmartmetersList(prosumerId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { prosumerId }, relations: ['smartmetersList'] });
  ...
  async findTradeorderscacheList(prosumerId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { prosumerId }, relations: ['tradeorderscacheList'] });
    const entities = parent?.tradeorderscacheList || [];
    // Convert entities to match GraphQL output format
    return entities.map(entity => ({
      ...entity,
      createdAtOnChain: entity.createdAtOnChain ? (entity.createdAtOnChain instanceof Date ? entity.createdAtOnChain.toISOString() : entity.createdAtOnChain) : null,
      updatedAtCache: entity.updatedAtCache ? (entity.updatedAtCache instanceof Date ? entity.updatedAtCache.toISOString() : entity.updatedAtCache) : null,
    }));
  }

  async findTransactionlogsList(prosumerId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { prosumerId }, relations: ['transactionlogsList'] });
  ...
  async findWalletsList(prosumerId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { prosumerId }, relations: ['walletsList'] });
