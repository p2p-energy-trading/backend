...
export class DeviceHeartbeatsService {
  ...
  async findAll(args?: DeviceHeartbeatsArgs): Promise<DeviceHeartbeats[]> {
    ...
    const where = {};
    if (args && args.heartbeatId !== undefined) where['heartbeatId'] = args.heartbeatId;
    if (args && args.meterId !== undefined) where['meterId'] = args.meterId;
    if (args && args.timestamp !== undefined) where['timestamp'] = args.timestamp;
    if (args && args.uptimeSeconds !== undefined) where['uptimeSeconds'] = args.uptimeSeconds;
    if (args && args.freeHeapBytes !== undefined) where['freeHeapBytes'] = args.freeHeapBytes;
    if (args && args.signalStrength !== undefined) where['signalStrength'] = args.signalStrength;
    if (args && args.additionalMetrics !== undefined) where['additionalMetrics'] = args.additionalMetrics;
  ...
  async findOne(heartbeatId: any): Promise<DeviceHeartbeats> {
    ...
    const entity = await this.repo.findOne({ where: { heartbeatId }, relations });
    if (!entity) {
      throw new Error(`DeviceHeartbeats with heartbeatId ${'$'}{heartbeatId} not found`);
    }
    return entity;
  }
  ...
  async update(heartbeatId: any, input: CreateDeviceHeartbeatsInput): Promise<DeviceHeartbeats> {
  ...
  async findSmartmeters(heartbeatId: any): Promise<any[]> {
    const parent = await this.repo.findOne({ where: { heartbeatId }, relations: ['smartmeters'] });
